...
const app = express();
const port = 2323;
app.get('/', (req, res) => res.send('WElcome Telah Aktif!'));

app.listen(port, () => console.log(`Welcome telah aktif dan membaca http://localhost:${port}`));
// KEEP ALIVE COMMANDS 
...
client.on("ready", () => {
  ...
  client.user.setActivity("saweria.co/kaixin!", { type: "PLAYING" });
});
...
client.on("guildMemberAdd", async (member) => {
  ...
  const welcomeMessage = (
    ...
    `\`${member.user.username}\` jangan lupa untuk menyapa teman-teman kita di <#857314564311351320>, untuk mengakrabkan diri dan bisa berbagi informasi tentang semua game yang kalian punya dan yang belum teman-teman KoF ketahui!`
  )
...
client.on("guildMemberRemove", async (member) => {
  ...
  const goodbyeMessage = (
    ...
    `\`${member.user.username}\` Buset ko keluar sih apa udah gak sayang sama teman-teman di dalam server ini!`
  )
    .replace(/<servername>/g, member.guild.name)
    .replace(/<membername>/g, member.user.username)
    .replace(/<membermention>/g, `<@${member.user.id}>`);
